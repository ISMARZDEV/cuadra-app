...
description: >
  ...
  goes to a human review queue: EAN → pg_trgm → pgvector(BGE-M3) → RRF → Codex-judge → queue. Owns
  ...
  provider, the Codex judge, or turning the SAVE_MATCHING_CASCADE_ENABLED flag on.
license: Apache-2.0
metadata:
  author: aispace
  version: "1.0"
...
- Tuning thresholds, adding a cascade stage, swapping the embedding model, or the Codex judge.
...
| Codex judge (fail-safe, structured output) | `contexts/save/infrastructure/matching/claude_judge.py` |
...
pg_trgm (léxico) + pgvector/BGE-M3 (semántico) → RRF (consenso) → boosts marca/tamaño → banding:
    ...
    MID [0.55, 0.85)                             → Codex-juez (banda gris)
    < MID  o  sin candidatos                     → cola humana (method=human)
Codex-juez {decision, confidence, cited_fields}:
