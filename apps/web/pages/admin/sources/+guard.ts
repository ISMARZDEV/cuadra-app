...
// Gate propio de `/admin/sources/*`: el `+guard.ts` del padre (`pages/admin/+guard.ts`) solo
...
// `admin_save_ingestion_ops` quedaría 403 en `/admin/sources` (mismo bug que motivó
// `pages/admin/providers/+guard.ts`). Re-chequea su propia capability.
