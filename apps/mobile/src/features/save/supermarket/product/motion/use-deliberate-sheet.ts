// PORTADO de la librería de patrones: `.claude/skills/motion-patterns/apps/grocery-concept/
// deliberate-return-sheet/src/motion/use-deliberate-sheet.ts`.
//
// ⚠️ Los números están MEDIDOS sobre el clip de referencia con kimógrafos a 60 fps, no
// elegidos a ojo. Antes de tocarlos, lee su `PATTERN.md`: la asimetría 150/420 ms ES el
// patrón, y simetrizarla rompe justo lo que se fue a medir.
...
export function useDeliberateSheet(open: boolean, { onClosed }: Options = {}) {
  ...
  // Mientras `onLayout` no haya medido, la hoja se esconde el viewport entero: con 0 de recorrido,
  // una hoja CERRADA estaría dibujada en su posición abierta durante el fotograma de montaje.
